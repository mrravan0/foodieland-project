...
const DetailsHero: FC<DetailsHeroProps> = ({ data }) => {
  ...
  return (
    <section className="py-20">
      <div className="container-custom flex flex-col gap-y-15">
        <div className="flex items-center justify-between max-tablet:flex-col max-tablet:gap-y-10">
          <div className="flex flex-col gap-y-12">
            <h1 className="text-custom-5xl">{state.title}</h1>
            <ul className="flex items-center justify-between gap-2.5 max-mobile:grid max-mobile:grid-cols-2 max-mobile:gap-10 max-[400px]:grid-cols-1">
              {data?.map((item) => (
                <li
                  className="not-first:after:bg-custom-grey-10 max-laptop:after:-left-1/3 max-mobile:after:content-none relative flex h-13 items-center gap-x-4 content-[''] after:absolute after:top-0 after:-left-1/2 after:h-full after:w-0.5 max-mobile:justify-evenly"
                  key={item.id}
                >
        ...
        />
        <p className="text-custom-grey-60 pt-5 max-laptop-s:text-center">
